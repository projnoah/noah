...
var map=Object.create(null);var shimmed=false;var isBrowserify=false; /**
 * Determine compatibility and apply patch.
 *
 * @param {Function} vue
 * @param {Boolean} browserify
 */exports.install=function(vue,browserify){if(shimmed)return;shimmed=true;Vue=vue;isBrowserify=browserify;exports.compatible=!!Vue.internalDirectives;if(!exports.compatible){console.warn('[HMR] vue-loader hot reload is only compatible with '+'Vue.js 1.0.0+.');return;} // patch view directive
patchView(Vue.internalDirectives.component);console.log('[HMR] Vue component hot reload shim applied.'); // shim router-view if present
var routerView=Vue.elementDirective('router-view');if(routerView){patchView(routerView);console.log('[HMR] vue-router <router-view> hot reload shim applied.');}}; /**
 * Shim the view directive (component or router-view).
 *
 * @param {Object} View
 */function patchView(View){var unbuild=View.unbuild;View.unbuild=function(defer){if(!this.hotUpdating){var prevComponent=this.childVM&&this.childVM.constructor;removeView(prevComponent,this); // defer = true means we are transitioning to a new
// Component. Register this new component to the list.
if(defer){addView(this.Component,this);}} // call original
return unbuild.call(this,defer);};} /**
 * Add a component view to a Component's hot list
 *
 * @param {Function} Component
 * @param {Directive} view - view directive instance
 */function addView(Component,view){var id=Component&&Component.options.hotID;if(id){if(!map[id]){map[id]={Component:Component,views:[],instances:[]};}map[id].views.push(view);}} /**
 * Remove a component view from a Component's hot list
 *
 * @param {Function} Component
 * @param {Directive} view - view directive instance
 */function removeView(Component,view){var id=Component&&Component.options.hotID;if(id){map[id].views.$remove(view);}} /**
 * Create a record for a hot module, which keeps track of its construcotr,
 * instnaces and views (component directives or router-views).
 *
 * @param {String} id
 * @param {Object} options
 */exports.createRecord=function(id,options){if(typeof options==='function'){options=options.options;}if(typeof options.el!=='string'&&_typeof(options.data)!=='object'){makeOptionsHot(id,options);map[id]={Component:null,views:[],instances:[]};}}; /**
 * Make a Component options object hot.
 *
 * @param {String} id
 * @param {Object} options
 */function makeOptionsHot(id,options){options.hotID=id;injectHook(options,'created',function(){var record=map[id];if(!record.Component){record.Component=this.constructor;}record.instances.push(this);});injectHook(options,'beforeDestroy',function(){map[id].instances.$remove(this);});} /**
 ...
 * @param {String} name
 * @param {Function} hook
 */function injectHook(options,name,hook){var existing=options[name];options[name]=existing?Array.isArray(existing)?existing.concat(hook):[existing,hook]:[hook];} /**
 * Update a hot component.
 *
 * @param {String} id
 * @param {Object|null} newOptions
 * @param {String|null} newTemplate
 */exports.update=function(id,newOptions,newTemplate){var record=map[id]; // force full-reload if an instance of the component is active but is not
// managed by a view
if(!record||record.instances.length&&!record.views.length){console.log('[HMR] Root or manually-mounted instance modified. Full reload may be required.');if(!isBrowserify){window.location.reload();}else { // browserify-hmr somehow sends incomplete bundle if we reload here
return;}}if(!isBrowserify){ // browserify-hmr already logs this
console.log('[HMR] Updating component: '+format(id));}var Component=record.Component; // update constructor
if(newOptions){ // in case the user exports a constructor
Component=record.Component=typeof newOptions==='function'?newOptions:Vue.extend(newOptions);makeOptionsHot(id,Component.options);}if(newTemplate){Component.options.template=newTemplate;} // handle recursive lookup
if(Component.options.name){Component.options.components[Component.options.name]=Component;} // reset constructor cached linker
Component.linker=null; // reload all views
record.views.forEach(function(view){updateView(view,Component);}); // flush devtools
if(window.__VUE_DEVTOOLS_GLOBAL_HOOK__){window.__VUE_DEVTOOLS_GLOBAL_HOOK__.emit('flush');}}; /**
 * Update a component view instance
 *
 * @param {Directive} view
 * @param {Function} Component
 */function updateView(view,Component){if(!view._bound){return;}view.Component=Component;view.hotUpdating=true; // disable transitions
view.vm._isCompiled=false; // save state
var state=extractState(view.childVM); // remount, make sure to disable keep-alive
var keepAlive=view.keepAlive;view.keepAlive=false;view.mountComponent();view.keepAlive=keepAlive; // restore state
restoreState(view.childVM,state,true); // re-eanble transitions
view.vm._isCompiled=true;view.hotUpdating=false;} /**
 ...
 * @param {Vue} vm
 * @param {Object} state
 */function restoreState(vm,state,isRoot){var oldAsyncConfig;if(isRoot){ // set Vue into sync mode during state rehydration
oldAsyncConfig=Vue.config.async;Vue.config.async=false;} // actual restore
if(isRoot||!vm._props){vm.$data=state.data;}else {Object.keys(state.data).forEach(function(key){if(!vm._props[key]){ // for non-root, only restore non-props fields
vm.$data[key]=state.data[key];}});} // verify child consistency
var hasSameChildren=vm.$children.every(function(c,i){return state.children[i]&&state.children[i].cid===c.constructor.cid;});if(hasSameChildren){ // rehydrate children
vm.$children.forEach(function(c,i){restoreState(c,state.children[i]);});}if(isRoot){Vue.config.async=oldAsyncConfig;}}function format(id){return id.match(/[^\/]+\.vue$/)[0];}},{}],3:[function(require,module,exports){(function(process,global){ /*!
 * Vue.js v1.0.20
 * (c) 2016 Evan You
...
/* istanbul ignore next */if(config.devtools){if(devtools){devtools.emit('init',Vue);}else if(process.env.NODE_ENV!=='production'&&inBrowser&&/Chrome\/\d+/.test(window.navigator.userAgent)){console.log('Download the Vue Devtools for a better development experience:\n'+'https://github.com/vuejs/vue-devtools');}}module.exports=Vue;}).call(this,require('_process'),typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{});},{"_process":1}],4:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:true});exports.default={computed:{title:function title(){return data.reset.title;},url:function url(){return data.reset.url;},placeholder:function placeholder(){return data.reset.placeholder;},button:function button(){return data.reset.button;},token:function token(){return document.querySelector('meta[name="_token"]').attributes['content'].nodeValue;}},methods:{ /**
         * Submit the password reset form
         * 提交重置密码表单
         *
         * @param e
         * @author Cali
         */resetPasswordSubmit:function resetPasswordSubmit(e){var form=e.target,self=this;this.setLoadingButton(false);$.post({url:form.action,data:$(form).serialize(),error:function error(_error){Noah.displayErrorAlert("出错了",_error.responseJSON.message);self.setLoadingButton(true);},success:function success(JSON){self.setLoadingButton(true);if(JSON.status==='succeeded'){Noah.displaySuccessAlert("成功发送",JSON.message);classie.removeClass(document.querySelector('#reset-dialog'),'dialog--open');}else {Noah.displayErrorAlert("出错了",JSON.message);}}});}, /**
         * Set the loading button.
         * 设置加载按钮
         *
         * @param done
         * @author Cali
         */setLoadingButton:function setLoadingButton(done){var button=this.$el.querySelector('button[type=submit]');done?$(button).removeAttr('disabled'):$(button).attr('disabled','disabled');button.innerHTML=done?'<i class="fa fa-envelope"></i>&nbsp;'+this.button:'<i class="fa fa-spin fa-circle-o-notch"></i>';}}};if(module.exports.__esModule)module.exports=module.exports.default;(typeof module.exports==="function"?module.exports.options:module.exports).template="\n<div id=\"reset-dialog\" class=\"dialog\">\n    <div class=\"dialog__overlay\"></div>\n    <div class=\"dialog__content\">\n        <h2>{{ title }}</h2>\n        <i class=\"fa fa-close dialog__close action\" data-dialog-close=\"\"></i>\n        <form role=\"form\" action=\"{{ url }}\" method=\"POST\" @submit.prevent=\"resetPasswordSubmit\">\n            <input type=\"hidden\" name=\"_token\" value=\"{{ token }}\">\n            <input type=\"email\" class=\"email\" name=\"email\" placeholder=\"{{ placeholder }}\" required=\"\" autocomplete=\"off\">\n            <button type=\"submit\" class=\"submit\"><i class=\"fa fa-envelope\"></i>&nbsp;{{ button }}</button>\n        </form>\n    </div>\n</div>\n";if(module.hot){(function(){module.hot.accept();var hotAPI=require("vue-hot-reload-api");hotAPI.install(require("vue"),true);if(!hotAPI.compatible)return;var id="/Users/calicastle1/Desktop/projnoah/resources/assets/js/pages/components/password-reset-dialog.vue";if(!module.hot.data){hotAPI.createRecord(id,module.exports);}else {hotAPI.update(id,module.exports,(typeof module.exports==="function"?module.exports.options:module.exports).template);}})();}},{"vue":3,"vue-hot-reload-api":2}],5:[function(require,module,exports){"use strict";var _passwordResetDialog=require("./components/password-reset-dialog.vue");var _passwordResetDialog2=_interopRequireDefault(_passwordResetDialog);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};} /*
 |------------------------------------------------------------
 ...
 | @author Cali
 |
 */var Vue=require('vue');$(function(){"use strict";var progressBtn=new UIProgressButton(document.querySelector('.progress-button')); // Trim declaration
if(!String.prototype.trim){(function(){var _this=this; // Make sure we trim BOM and NBSP
var trim=/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g;String.prototype.trim=function(){return _this.replace(trim,'');};})();} // Setup the App object
var App={ /**
         ...
         *
         * @author Cali
         */initSetup:function initSetup(){[].slice.call(document.querySelectorAll('input.input__field')).forEach(function(inputEl){ // In case the input is already filled..
if(inputEl.value.trim()!==''){classie.add(inputEl.parentNode,'input--filled');} // Bind events:
inputEl.addEventListener('focus',App.onInputFocus);inputEl.addEventListener('blur',App.onInputBlur);});this.blurBackgroundTypingPassword();this.submitWhenHitsEnter();this.setupDialog();}, /**
         * Focus input event listener
         * 输入焦点事件处理
         *
         * @author Cali
         */onInputFocus:function onInputFocus(ev){classie.add(ev.target.parentNode,'input--filled');}, /**
         * Blur input event listener
         * 输入离开事件处理
         *
         * @author Cali
         */onInputBlur:function onInputBlur(ev){if(ev.target.value.trim()===''){classie.remove(ev.target.parentNode,'input--filled');}}, /**
         * Apply blur filter when typing in password
         * 输入密码时增加高斯模糊图层
         *
         * @author Cali
         */blurBackgroundTypingPassword:function blurBackgroundTypingPassword(){ // When the password input got focused, let's blur out the background
$("input#input-password").focus(function(){return $('body').addClass("blur-out");}).blur(function(){return $('body').removeClass("blur-out");});}, /**
         * Bind the enter key to form submission
         * 绑定回车提交表单
         *
         * @author Cali
         */submitWhenHitsEnter:function submitWhenHitsEnter(){$("#panel input").keydown(function(e){if(e.keyCode===13){$(progressBtn.el.querySelector('button')).click();}});}, /**
         * New up dialog and its event listeners
         * 新建对话框与事件响应
         *
         * @author Cali
         */setupDialog:function setupDialog(){var dialogTrigger=document.querySelector('[data-dialog]'),dialog=document.getElementById(dialogTrigger.getAttribute('data-dialog')),dlg=new DialogFx(dialog);dialogTrigger.addEventListener('click',dlg.toggle.bind(dlg));}, /**
         * Submit the form using AJAX
         * AJAX提交表单
         *
         * @returns {boolean}
         * @author Cali
         */submit:function submit(){var _this2=this;var has_empty_field=false;$('input.input__field--login').each(function(){if($(this).val().trim()===''){$(this).focus();has_empty_field=true;return false;}}); // If any field is empty
if(has_empty_field||this.buttonLock)return false;this.buttonLock=true; // Let's send a request to login
setTimeout(function(){return _this2.sendLoginRequest(_this2.$form);},200);}, /**
         * Send request to server
         * 发送服务器请求
         *
         * @param form {Object}
         * @author Cali
         */sendLoginRequest:function sendLoginRequest(form){var _this3=this;$.post({url:$(form).attr('action'),data:$(form).serializeArray(),error:function error(_error){_this3.buttonLock=false;_this3.setLoginStatus(false);console.log(JSON.parse(_error.responseText));},success:function success(JSON){_this3.buttonLock=false;if(JSON.status=='succeeded'){ // Login success
_this3.setLoginStatus(true); // Redirect back to the intended url
setTimeout(function(){return window.location.href=JSON.redirect;},1000);}else { // Login failed
_this3.setLoginStatus(false); // Handle error message
_this3.handleError(JSON);}}});}, /**
         ...
         * @author Cali
         */notify:function notify(errorMsg){var notification=new TopBarNotify('<span class="fa fa-times"></span><p>'+errorMsg+'</p>','warning',function(){});notification.display();}, /**
         * Handle error message and display a notification
         * 验证错误时处理 与 显示提醒消息
         *
         * @param JSON {Object}
         * @author Cali
         */handleError:function handleError(JSON){var errorMsg="";if(this.vm().isRegister){var messages=JSON.messages;if(messages.hasOwnProperty('email')){errorMsg=messages.email;}else if(messages.hasOwnProperty('password')){errorMsg=messages.password;}else {errorMsg=messages.username;}}else {errorMsg=JSON.error;} // Create the notification
this.notify(errorMsg);}, /**
         ...
         * @param status {int}
         * @author Cali
         */setLoginStatus:function setLoginStatus(status){progressBtn.setProgress(1);progressBtn.stop(status?1:-1);}, /**
         * Setup the V-M Vue singleton
         * 设置 视图-模型 单例
         *
         * @returns {Vue Object}
         */vm:function vm(){return _vm?_vm:new Vue({el:'#app',components:{PasswordResetDialog:_passwordResetDialog2.default},data:{isRegister:data.isRegister=='true'?true:false},ready:function ready(){$("#remember_me_cb").iCheck();},computed:{title:function title(){return this.isRegister?data.register.title:data.login.title;},url:function url(){return this.isRegister?data.register.url:data.login.url;},buttonTitle:function buttonTitle(){return this.isRegister?data.register.title:data.login.title;},inputs:function inputs(){return this.isRegister?data.register.inputs:data.login.inputs;},switchButtonTitle:function switchButtonTitle(){return !this.isRegister?data.register.title:data.login.title;}},methods:{ /**
                     * Toggle between register and login form
                     * 登录/注册表单切换
                     *
                     * @author Cali
                     */toggleForm:function toggleForm(){this.isRegister=!this.isRegister;this.substituteTitle();setTimeout(function(){return App.initSetup();},300);}, /**
                     * Submit the main form
                     * 提交主要表单
                     *
                     * @author Cali
                     */submitForm:function submitForm(){App.submit();}, /**
                     * Substitute the document title accordingly
                     * 替换文件标题
                     *
                     * @author Cali
                     */substituteTitle:function substituteTitle(){document.title=this.title+" "+Site.title;}}});}}; // Get Vue
var _vm=App.vm();_vm.substituteTitle(); // Wait for some animations
setTimeout(function(){return App.initSetup();},300);});},{"./components/password-reset-dialog.vue":4,"vue":3}]},{},[5]); //# sourceMappingURL=login.es6.js.map
